# %%
"""
Script to extract and save learned features from UPD data from the bottleneck layer of the USleep model
Features are extracted and saved as the whole healthy and MCI datasets
"""
...
# %%
# paths to test data
healthy_path = '../data_preparation/prepared_data_upd/healthy'
mci_path = '../data_preparation/prepared_data_upd/mci'

# prepare test data
# %%
# load mean and variance of normalizer (adapted on train set)
mean = np.load('../pretext_task/normalizer_upd/normalizer_mean.npy')
variance = np.load('../pretext_task/normalizer_upd/normalizer_variance.npy')
...
# %%
# Load saved datasets
healthy = tf.data.experimental.load(healthy_path)
mci = tf.data.experimental.load(mci_path)

# one hot encode labels and transpose signals
def preprocessing(signals, labels, age, moca):
    labels = tf.one_hot(labels, depth=num_classes)
    signals = tf.transpose(signals, perm=[1, 0])
    return signals, labels, age, moca

# %%
# USE FUNCTIONS TO PREPARE DATA
healthy = healthy.map(preprocessing)
healthy = healthy.batch(batch_size)
healthy = healthy.map(lambda signal, labels, age, moca: (normalizer(signal), labels, age, moca))

mci = mci.map(preprocessing)
mci = mci.batch(batch_size)
mci = mci.map(lambda signal, labels, age, moca: (normalizer(signal), labels, age, moca))

# %%
# load save weights and create model
model = MyUSleep(num_classes=num_classes, in_shape=(None, in_channels))
model.load_weights('../pretext_task/weights/trained_upd/20250428-142901_trained_usleep_weights_normal')

timepoints_n = int(100 * 60 * 3 * 60) # 3 hours of data
# Create a dummy input tensor
dummy_input = tf.random.normal(shape=(batch_size, timepoints_n, in_channels))  # Adjust shape as needed
...
datasets = {"healthy": healthy, "mci": mci}
...
for dataset_name, dataset in datasets.items():
    ...
    for signals, labels, age, moca in dataset:
        # Extract bottleneck features
        _,  bottleneck_features = model(signals, training=False)

        features_list.append(bottleneck_features)
        ages_np = age.numpy().reshape(-1)
        ages_list.extend(ages_np)
        mocas_np = moca.numpy().reshape(-1)
        mocas_list.extend(mocas_np)
    ...
    mocas[f"{dataset_name}"] = np.array(mocas_list)

    # Check the shapes of the extracted features, mocas and ages
    print(f"{dataset_name} features shape: {features[dataset_name].shape}")
    print(f"{dataset_name} ages shape: {ages[dataset_name].shape}")
    print(f"{dataset_name} mocas shape: {mocas[dataset_name].shape}")
...
# %%
# save features and ages
features_dir = 'features_upd'
...
for dataset_name in averaged_features.keys():
    ...
    dataset_mocas = mocas[f"{dataset_name}"]

    # create DataFrame
    df1 = pd.DataFrame(dataset_features)
    df1['age'] = dataset_ages
    df1['moca'] = dataset_mocas
